...
package com.tal.bigdata.binlog.deliver.truncate;
    ...
    public void truncate(Path filePath, long length) throws IOException {
        ...
        if (fs.exists(filePath)) {
            ...
            if (length == 0L) {
                ...
                System.out.println(filePath+ "  执行truncate file操作: 源文件size = "+fileSize+" 目标文件size= "+ length);
                copyFileContent(filePath, truncatedFilePath, length);

                if (length !=  fs.getFileStatus(truncatedFilePath).getLen()) {
                    throw new RuntimeException(filePath+  "  Did not truncate to right length. Should be " + length + " is " + fs.getFileStatus(truncatedFilePath).getLen() + ".");
                }
                fs.delete(filePath, false);
