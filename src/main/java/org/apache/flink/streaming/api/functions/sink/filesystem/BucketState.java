...
class BucketState<BucketID> {
    ...
    public String toString() {
        ...
        if (hasInProgressResumableFile()) {
            HadoopFsRecoverable inProgressResumableFile = (HadoopFsRecoverable) this.inProgressResumableFile;
            strBuilder.append(", has open part file "+inProgressResumableFile.toString()+" created  @ ").append(inProgressFileCreationTime);
        }
        ...
        if (!committableFilesPerCheckpoint.isEmpty()) {
            ...
            for (long checkpointId: committableFilesPerCheckpoint.keySet()) {
                List<RecoverableWriter.CommitRecoverable> commitRecoverables = committableFilesPerCheckpoint.get(checkpointId);
                RecoverableWriter.CommitRecoverable commitRecoverable = commitRecoverables.get(0);
                HadoopFsRecoverable commitRecoverable1 = (HadoopFsRecoverable) commitRecoverable;

                strBuilder.append(checkpointId).append(' ');
                strBuilder.append(commitRecoverable1.toString());
