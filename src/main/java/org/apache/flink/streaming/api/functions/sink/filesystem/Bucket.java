...
public class Bucket<IN, BucketID> {
    ...
    private Bucket(
        ...
        this(
                ...
                partFileFactory,
                rollingPolicy);
        restoreInProgressFile(bucketState); //恢复上次checkpoint的inprocess状态的临时文件的inProgressPart对象
        commitRecoveredPendingFiles(bucketState); //处理上次checkpoint的pending文件，就是check pending文件转换成的正式文件是否正确
    }
    private void restoreInProgressFile(final BucketState<BucketID> state) throws IOException {
        ...
        //该对象resumable包含3个属性 1、临时文件的tmpfile名称  2、临时文件的targetfile名称  3、临时文件的offset
        final ResumeRecoverable resumable = state.getInProgressResumableFile();
        if (fsWriter.supportsResume()) {
            //stream可以打开tmpfile名称的临时文件，把offset恢复至resumable的offset，然后继续写入
            final RecoverableFsDataOutputStream stream = fsWriter.recover(resumable);
            //inProgressPart 封装了stream，可以临时文件继续上次的offset继续写入
    ...
    void write(IN element, long currentTime) throws IOException {
        //首次写入数据inProgressPart（最终的写入操作对象，封装了FsDataOutputStream对象）为null  ||  达到了开启写入新文件的条件
    ...
    private CommitRecoverable closePartFile() throws IOException {
        ...
        if (inProgressPart != null) {
            //当前临时文件的数据持久化到disk，committable封装了临时文件的tmpfile、targetfile和offset
            committable = inProgressPart.closeForCommit();
            //当前临时文件状态转为pending状态，并把committable放入list中，在checkpoint时会存储pendingPartsForCurrentCheckpoint的state
            pendingPartsForCurrentCheckpoint.add(committable);
            inProgressPart = null;
        }
        return committable;
    ...
    //当前的inprocess文件属性和pending状态的属性封装到BucketState对象中
    BucketState<BucketID> onReceptionOfCheckpoint(long checkpointId) throws IOException {
        //判断是否可以关闭inprocess的临时文件
        //将pending状态的文件放入pendingPartsPerCheckpoint属性中
